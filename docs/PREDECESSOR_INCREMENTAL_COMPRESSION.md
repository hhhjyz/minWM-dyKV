...
旧推理入口曾将 `viewmats/Ks` 转成 BF16，约 `1.5e-3` 的纯 yaw 矩阵误差无法通过
`_pure_yaw_delta` 的 `1e-4` 容差，已有 predecessor 视频因而全部进入
`predecessor_fixed_novelty_fallback`。当前版本已让 dyKV 相机几何保持 FP32，并只在 PRoPE
算子内部转换私有副本；与推理入口一致的 `j*7` 回归测试已进入
`predecessor_incremental_yaw` 的 `1/4` 档。旧视频仍需重新生成，正式实验也必须核验日志。
完整调用链、修复和验收字段见

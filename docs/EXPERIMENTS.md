...
| 运行编号 | 方法 | 目的 | 状态 |
| --- | --- | --- | --- |
| B0 | 上游 minWM local cache | 质量/速度基线 | 待运行 |
| B1 | 不压缩的 dyKV | 单独分析检索收益 | 待运行 |
| B2 | 完整 dyKV | 评估记忆、速度与质量 | 待运行 |

`B1` 是 Python 层的诊断预设，不新增公开命令行接口。
...
- B0/B1/B2 必须使用相同的用例分配、checkpoint、latent 长度、分辨率和 seed。
...
| 待运行 | B2 | 待运行 | 0 | 待运行 | 待运行 | 待运行 | 待运行 | 完整方法 |
...
因此不报告该数值。正式 B0/B1/B2 和 MBench 结果仍保持“待运行”，直到完成对应实验。

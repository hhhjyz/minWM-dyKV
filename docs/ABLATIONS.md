...
代码内部支持三种 `DyKVConfig.compression_mode`，不增加公开 CLI 参数：

| 编号 | dyKV | `compression_mode` | 方法 | 回答的问题 |
| --- | --- | --- | --- | --- |
| A0 | 关闭 | — | 上游 local cache | 无长期记忆的基线表现 |
| A1 | 开启 | `none` | FOV 检索，不压缩 retrieval KV | 检索本身带来的质量收益与最大开销 |
| A2 | 开启 | `fixed_novelty` | 固定锚点 + 新颖性压缩 | 与相机无关的内容压缩效果 |
| A3 | 开启 | `yaw_fov` | yaw/FOV 动态空间裁剪 | 完整方法是否取得更好的质量/效率折中 |

正式推理默认 A3。A1/A2 是 Python/配置层诊断预设，可在实验配置中设置
`dykv_compression_mode: none` 或 `fixed_novelty`，不暴露新的公开命令行超参数。
...
| A8 | `K` 推导 FOV / 固定 60° | 其余相同 | 验证真实内参对裁剪边界的影响 |

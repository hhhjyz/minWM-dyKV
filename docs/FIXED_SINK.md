...
```text
baseline:
[ fixed sink: 4 ][ rolling local including current: 16 ]
...
固定 sink 内容和 sink-only RoPE rebase 是两个不同概念：

- 本项目所有 case 都固定保留最初 4 帧；
- 不启用 `minWM-back` 中独立的 `fixed_sink_rope_rebase`；
- baseline 保持普通 RoPE 路径；
- dyKV 的三区域 RoPE 中，sink K 仍位于训练位置 0--3，retrieval 映射到 4--11，local
  与 query 映射到 12--19。

这样不会把“是否固定 sink”和“是否重映射 sink”混成同一个实验变量。

## 4. 当前 Case

| Case | Sink | 其余布局 |
| --- | --- | --- |
| `baseline` | 固定最初 4 帧 | rolling local 16 |

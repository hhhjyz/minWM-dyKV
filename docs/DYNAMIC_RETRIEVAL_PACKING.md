...
阈值是第一版内部预设，不作为新的公开连续超参数。后续只把档位表作为有限消融 case
替换，例如 `{1, 1/2, 1/4}` 对比 `{1, 3/4, 1/2, 1/4}`。
...
5. 平移、pitch、roll 或非法内参继续走固定新颖性回退，但回退结果也必须量化到同一档位。
...
在 E0--E3 和四个典型 MBench 样本证明 slot sharing 不显著损害质量后，再考虑精确任意比例
背包或更多档位。
